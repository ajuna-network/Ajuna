...
use ajuna_service::{
	...
	chain_spec, para_bajun as para,
	para_bajun::BajunRuntimeExecutor,
...
fn load_spec(id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
	Ok(match id {
		...
		path => Box::new(chain_spec::bajun::ChainSpec::from_json_file(PathBuf::from(path))?),
	})
}
...
pub fn run() -> sc_cli::Result<()> {
	...
	match &cli.subcommand {
		...
		Some(_) => Err("Unsupported command for bajun parachain".into()),
		None => {
