...
fn validate_genesis_vesting_schedules() {
	ExtBuilder::default().existential_deposit(MICRO_AJUNS).build().execute_with(|| {
		...
		assert_eq!(alice_free_balance, MICRO_AJUNS * 10_000); // Account 1 has free balance
		assert_eq!(bob_free_balance, MICRO_AJUNS * 20_000); // Account 2 has free balance
		assert_eq!(charlie_free_balance, MICRO_AJUNS * 30_000); // Account 12 has free balance

		let alice_vesting = VestingSchedule {
			start: 0,
			period: 1,
			period_count: 1_000,
			per_period: 5 * MICRO_AJUNS,
		};
		assert_eq!(
...
fn validate_simple_vesting_schedule() {
	ExtBuilder::default()
		.existential_deposit(MICRO_AJUNS)
...
fn validate_complex_vesting_schedule() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// We validate that for each block starting at block 110, 450 µAJUNS become claimable
			// into BOB's account
			...
			// Finally at block 2110 all vested tokens are reclaimed and no more AJUNS are locked
			// in BOB's account
