...
fn validate_genesis_vesting_schedules() {
	ExtBuilder::default()
		.existential_deposit(1_000 * AJUN)
		.build()
		.execute_with(|| {
			...
			assert_eq!(alice_free_balance, 1_000 * AJUN * 10_000); // Account 1 has free balance
			assert_eq!(bob_free_balance, 1_000 * AJUN * 20_000); // Account 2 has free balance
			assert_eq!(charlie_free_balance, 1_000 * AJUN * 30_000); // Account 12 has free balance

			let alice_vesting = VestingSchedule {
				start: 0,
				period: 1,
				period_count: 1_000,
				per_period: 5_000 * AJUN,
			};
			assert_eq!(
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let vesting_schedule =
				VestingSchedule { start: 10, period: 10, period_count: 5, per_period: 15 * AJUN };
...
fn validate_complex_vesting_schedule() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let cliff_vesting_schedule =
				VestingSchedule { start: 10, period: 100, period_count: 1, per_period: 100 * AJUN };
			...
			// We assert that BOB has 1000 AJUN locked
			assert_eq!(
			...
			// We assert that BOB has still 1000 AJUN locked
			assert_eq!(
				Balances::locks(bob_id.clone()).get(0),
				Some(&BalanceLock {
					id: VESTING_LOCK_ID,
			...
			// We assert that BOB has now only 900 AJUN locked
			assert_eq!(
				Balances::locks(bob_id.clone()).get(0),
				Some(&BalanceLock {
					id: VESTING_LOCK_ID,
			...
			// We validate that for each block starting at block 110, 900 mAJUN become claimable
			// into BOB's account
			for i in (111..=1109).step_by(100) {
				...
				assert_eq!(
					...
					Some(&BalanceLock {
						id: VESTING_LOCK_ID,
						amount: (900 * AJUN) - (900 * MILLI_AJUN * (i as u128 - 110)),
						reasons: Reasons::All,
					})
				);
			}

			// Finally at block 2110 all vested tokens are reclaimed and no more AJUN are locked
			// in BOB's account
